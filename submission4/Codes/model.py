...
mat = scipy.io.loadmat('All_PPCA.mat')
ppca = mat['ppca'].astype('float64')

ppca = ppca[:,0:100,:] 
mat1 = scipy.io.loadmat('mean_all_images.mat')
mu = mat1['mean']


#ppca = ppca[:,0:5,:]

#%mu = mat['mean'].astype('float64')
...
""")
    ...
    def generator(self, z):
    	...
    	#Here gf_dim will decide the no of nuerons 

        self.z_, self.h0_w, self.h0_b = linear(z, 100*3, 'g_h0_lin', with_w=True)
	self.h0 = tf.reshape(self.z_, [-1, 100, 1, 3])
	
	h0 = tf.nn.relu(self.g_bn0(self.h0))
		
	
	#output = tf.get_variable("Generate", [self.batch_size,4096,3], tf.float32)
        #output = tf.zeros([self.batch_size,4096,3])
        #output = np.empty([self.batch_size,4096,3],dtype='float32')
	final = tf.zeros([0,64,64,3])

	for i in range(self.batch_size):
		output = multi(h0,tf.cast(ppca,tf.float32),tf.cast(mu,tf.float32),i)
		output = tf.reshape(output,[1,64,64,3])
		final = tf.concat([final,output],0)	
    ...
    def sampler(self, z, y=None):
        ...
        h0 = tf.reshape(linear(z,100*3,'g_h0_lin'),
                        [-1, 100, 1, 3])
